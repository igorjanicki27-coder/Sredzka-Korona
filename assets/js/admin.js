(function () {
  ...
  function collectMenuFromPanel() {
    const menu = [];
    const sections = document.querySelectorAll("[data-menu-section-name]");
    sections.forEach((sectionInput, sectionIndex) => {
      const sectionName = sectionInput.value.trim();
      if (!sectionName) return;
      
      // Zbierz mapowanie starych nazw podkategorii na nowe
      const subcategoryMapping = {};
      document.querySelectorAll(`[data-menu-subcategory-name^="${sectionIndex}-"]`).forEach(input => {
      ...
      const items = [];
      const itemInputs = document.querySelectorAll(`[data-menu-item-name^="${sectionIndex}-"]`);
      itemInputs.forEach((itemInput) => {
        ...
        const price = document.querySelector(`[data-menu-item-price="${sectionIndex}-${itemIdx}"]`)?.value.trim() || "";
        const description = document.querySelector(`[data-menu-item-description="${sectionIndex}-${itemIdx}"]`)?.value.trim() || "";
        let subcategory = document.querySelector(`[data-menu-item-subcategory="${sectionIndex}-${itemIdx}"]`)?.value.trim() || "";
        
        // Zastosuj mapowanie nazw podkategorii
        if (subcategory && subcategoryMapping[subcategory]) {
          subcategory = subcategoryMapping[subcategory];
        }
        
        const ingredientsText = document.querySelector(`[data-menu-item-ingredients="${sectionIndex}-${itemIdx}"]`)?.value || "";
