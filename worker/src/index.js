  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/admin/login" && request.method === "POST") {
        const payload = await request.json();
        return handleAdminLogin(payload, request, env);
      }
...
async function handleAdminLogin(payload, request, env) {
  if (!payload.login || !payload.password) {
    return jsonResponse({ error: "Login i haslo sa wymagane." }, 400, request, env);
  }
  const loginHash = await digest(`${env.AUTH_PEPPER}:${payload.login}`);
  const passwordHash = await digest(`${env.AUTH_PEPPER}:${payload.password}`);
  if (
    !constantTimeEqual(loginHash, env.ADMIN_LOGIN_HASH) ||
    !constantTimeEqual(passwordHash, env.ADMIN_PASSWORD_HASH)
  ) {
    return jsonResponse({ error: "Nieprawidlowy login lub haslo." }, 401, request, env);
  }
  const rawToken = crypto.randomUUID() + crypto.randomUUID();
  const tokenHash = await digest(rawToken);
  await env.DB.prepare(
    "INSERT INTO admin_sessions (token_hash, created_at, last_seen, revoked_at) VALUES (?, ?, ?, NULL)"
  )
    .bind(tokenHash, nowIso(), nowIso())
    .run();
  return jsonResponse(
    { ok: true },
    200,
    request,
    env,
    { "Set-Cookie": buildSessionCookie(request, rawToken) }
  );
}

async function requireSession(request, env) {
  const rawToken =
    parseCookies(request.headers.get("Cookie") || "")[sessionCookieName(request)];
  if (!rawToken) {
    throw unauthorized("Brak aktywnej sesji administratora.");
  }
  const tokenHash = await digest(rawToken);
  const session = await env.DB.prepare(
    "SELECT token_hash AS tokenHash FROM admin_sessions WHERE token_hash = ? AND revoked_at IS NULL"
  )
    .bind(tokenHash)
    .first();
  if (!session) {
    throw unauthorized("Sesja nie jest prawidlowa.");
  }
  await env.DB.prepare(
...
function sessionCookieName(request) {
  const url = new URL(request.url);
  return url.protocol === "https:" ? `__Host-${SESSION_COOKIE}` : SESSION_COOKIE;
}

function buildSessionCookie(request, token) {
  const secure = new URL(request.url).protocol === "https:";
  const flags = [`${sessionCookieName(request)}=${token}`, "Path=/", "HttpOnly", "SameSite=Strict"];
  if (secure) {
    flags.push("Secure");
  }
...
function clearSessionCookie(request) {
  const secure = new URL(request.url).protocol === "https:";
  const flags = [`${sessionCookieName(request)}=`, "Path=/", "HttpOnly", "SameSite=Strict"];
  if (secure) {
    flags.push("Secure");
  }
